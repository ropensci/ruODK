...
test_that("odata_submissions_get skip omits number of results", {
  fresh_raw <- odata_submissions_get(
    Sys.getenv("ODKC_TEST_PID"),
    Sys.getenv("ODKC_TEST_FID"),
    url = Sys.getenv("ODKC_TEST_URL"),
  ...
  skip_raw <- odata_submissions_get(
    Sys.getenv("ODKC_TEST_PID"),
    Sys.getenv("ODKC_TEST_FID"),
    skip = 1,
    url = Sys.getenv("ODKC_TEST_URL"),
  ...
  skip_parsed <- skip_raw %>% parse_submissions()

  testthat::expect_true(nrow(fresh_parsed) == nrow(skip_parsed) + 1)
})

test_that("odata_submissions_get top limits number of results", {
  top_raw <- odata_submissions_get(
    Sys.getenv("ODKC_TEST_PID"),
    Sys.getenv("ODKC_TEST_FID"),
    top = 1,
    url = Sys.getenv("ODKC_TEST_URL"),
  ...
  top_parsed <- top_raw %>% parse_submissions()

  testthat::expect_true(nrow(top_parsed) == 1)
})


test_that("odata_submissions_get count returns total number or rows", {
  x_raw <- odata_submissions_get(
    Sys.getenv("ODKC_TEST_PID"),
    Sys.getenv("ODKC_TEST_FID"),
    count = TRUE,
    top = 1,
  ...
  x_parsed <- x_raw %>% parse_submissions()
