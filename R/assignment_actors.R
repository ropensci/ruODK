...
#' @template param-tz
#' @return A tibble with one row for each Actor assigned the Role as per the
#'   ODK Central API docs.
#'   Column names are renamed from ODK's `camelCase` to `snake_case`.
# nolint start
#' @seealso \url{https://docs.getodk.org/central-api-accounts-and-users/#listing-all-actors-assigned-some-role}
# nolint end
...
assignment_actors <- function(
  ...
  if (odkc_version |> semver_lt("0.5")) {
    ru_msg_warn("assignment_actors is supported from v0.5")
  }
...
# usethis::use_test("assignment_actors")  # nolint
