...
data <- data_main_raw %>% 
  ...
  dplyr::mutate(
    my_photo = ruODK::attachment_get(id, my_photo, local_dir = loc),
    another_photo = ruODK::attachment_get(id, another_photo, local_dir = loc)
...
data_sub1 <- data_sub1_raw %>% 
  ...
  ruODK::parse_datetime(tz = tz, col_contains = "date") %>%
  dplyr::mutate(
    some_photo = ruODK::attachment_get(id, some_photo, local_dir = loc),
    # add for each form field of type media attachment
  ) %>%
...
* Upload same data, report, attachments to Google Drive.
...
# Run once to create resources on an existing dataset
d <- ckanr::package_show("dataset-title")
res_data_main <- ckanr::resource_create(
  package_id = d$id, title="Main data", upload = data_fn)
res_data_sub1 <- ckanr::resource_create(
  package_id = d$id, title="Nested data table 1", upload = data_sub1_fn)
res_report <- ckanr::resource_create(
  package_id = d$id, title="Nested data table 1", upload = rep_fn)
res_zip <- ckanr::resource_create(
  package_id = d$id, title="All data and attachments", upload = zip_fn)
...
# Google Drive
#
# Once per machine, then comment out:
