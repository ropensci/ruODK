...
recursively unnest list columns using `tidyr::unnest_wider()`.
...
This behaviour can be disabled by handing the argument `names_sep=None` to
`tidyr::unnest_wider()` through running 
`ruODK::odata_submission_get() %>% ruODK::odata_submission_rectangle(names_sep = None)`.
...
`ruODK::ru_datetime()`. We use our local timezone (GMT+08) in this example.
When used through `ruODK::odata_submission_get()`, `ruODK` will introspect the
...
fully) specified, see `??ruODK::ru_datetime()`.
...
receive GeoJSON. Point GeoJSON parses with `odata_submission_get()` into separate 
columns for `lat`, `lon`, `alt` (soon to include accuracy). 
