...
names(data_quadrat_raw) <- names(data_quadrat_raw) %>% stringr::str_replace_all(., "-", "_")
names(data_taxon_raw) <- names(data_taxon_raw) %>% stringr::str_replace_all(., "-", "_")
...
The structure is different to the output of `ruODK::odata_submissions_get`,
therefore `parse_submissions` does not work (yet) for those.

```{r}
d <- submissions %>% purrr::map(., c("xml", "data")) 
d
# that's as far as it goes for today, stay tuned
```
