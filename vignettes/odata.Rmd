...
This URL contains the base URL (`r url`), the project ID (`r pid`), and the form ID (`r fid`).

ruODK made the opinionated decision to use these components separately in its functions.
...
Repeating form groups, in our example "taxon_encounter", are named "Submissions.taxon_encounter". 
The exact name is found under DataServices.Schema.EntityType[n].attrs.Name.
...
```{r load_odata, eval=F}
fq_raw <- odata_submissions_get(pid, fid, url = url, un = un, pw = pw)
fq_tae <- odata_submissions_get(pid, fid, url = url, un = un, pw = pw, 
...
# Rectangle the data
The function `ruODK::odata_submissions_get` returned the original XML response as a 
...
fq_data <- fq_raw %>% parse_submissions(verbose=TRUE) %>% 
  ...
  dplyr::mutate(
    quadrat_photo = attachment_get(pid, fid, uuid, quadrat_photo, url = url),
    morphological_type_photo = attachment_get(pid, fid, uuid, morphological_type_photo, url = url),
    mudmap_photo = attachment_get(pid, fid, uuid, mudmap_photo, url = url),
...
fq_data_tae <- fq_tae %>% 
  ...
  dplyr::mutate(
    photo_in_situ = attachment_get(pid, fid, uuid, photo_in_situ, url = url)
  ) %>% 
...
If we run `odata_submissions_get()` with `wkt=FALSE` (the default), we will
...
If we run `odata_submissions_get()` with `wkt=TRUE` (the default), we will
