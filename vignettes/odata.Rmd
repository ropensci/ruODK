...
and links file attachments. For the fun of it, we'll enable verbose messages and
watch `ruODK` churning the data.

The only manual step remaining is to rename the point location components.
...
fq_data_strata <- ruODK::odata_submission_get(
  table = fq_svc$name[2], verbose = TRUE, tz = tz, local_dir = loc
)

fq_data_taxa <- ruODK::odata_submission_get(
  table = fq_svc$name[3], verbose = TRUE, tz = tz, local_dir = loc
  ) %>%
  dplyr::rename(
    lon = x6,
    lat = x7,
...
Fields of type "binary" (attachments) are listed in the form schema, `fq_fs`.
...
Fields of type "dateTime" and "date" are listed in the form schema, `fq_fs`.

The trailing `invisible` is a neutral operation to terminate trailing magrittr 
...
`fq_taxa` (repeated group "Taxon Encounter" within a Flora Quadrat), and
`fq_strata` (repeated group "Vegetation Stratum" within a Flora Quadrat).
...
# Do it yourself
To reproduce the demonstrated workflow easily in RStudio, create a new RMarkdown
document "From template" and select ruODK's template "odata", or run 
`rmarkdown::draft("test.Rmd", "odata", package="ruODK")`.
