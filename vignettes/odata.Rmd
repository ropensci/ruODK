...
fq_meta <- get_metadata(pid, fid, url = url, un = un, pw = pw)
```
...
```{r load_odata, eval=F}
fq_raw <- get_submissions(pid, fid, url = url, un = un, pw = pw)
fq_tae <- get_submissions(pid, fid, url = url, un = un, pw = pw, sub="Submissions.taxon_encounter")
```
...
fq_data <- fq_raw %>% parse_submissions(verbose=TRUE) %>% 
  ...
  dplyr::mutate(
    quadrat_photo = get_attachment(pid, fid, uuid, quadrat_photo, url = url),
    morphological_type_photo = get_attachment(pid, fid, uuid, morphological_type_photo, url = url),
    mudmap_photo = get_attachment(pid, fid, uuid, mudmap_photo, url = url),
...
fq_data_tae <- fq_tae %>% 
  ...
  dplyr::mutate(
    photo_in_situ = get_attachment(pid, fid, uuid, photo_in_situ, url = url)
  ) %>% 
