...
fq_fs %>% dplyr::filter(type %in% c("dateTime", "date")) %>% knitr::kable(.)
# Candidates for ruODK::attachment_get()
fq_fs %>% dplyr::filter(type == "binary") %>% knitr::kable(.)
```
...
data("fq_data")
data("fq_data_taxa")
fq_data_taxa_joined <- fq_data_taxa %>%
  dplyr::left_join(fq_data, by = c("submissions_id" = "id"))
