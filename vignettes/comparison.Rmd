...
| External dependencies        | None | Java, ODK Briefcase | SPSS | Stata | Java, ODK Briefcase, wraps `odkr` | [Pentaho Kettle](https://github.com/pentaho/pentaho-kettle), Java |
...
## Related tools beyond R

Two non-R tools cover the same ODK Central API and deserve a mention.

[`pyODK`](https://getodk.github.io/pyodk/) is the official ODK Python client
for the ODK Central API (`pip install pyodk`, targets Central v2026.2.0).
It offers a `Client` with `projects`, `forms`, `submissions`, `entities` and
`entity_lists` helpers, plus generic HTTP verb methods that expose the full
API without a dedicated helper.
`pyODK` is the Python counterpart to `ruODK`: scripted, reproducible
pipelines, with tabular Submission data via `get_table()`.
Choose between them by pipeline language, not by capability.

[`bruno-API-ODKCentral`](https://github.com/CEN-Nouvelle-Aquitaine/bruno-API-ODKCentral)
is a ready-made collection for [Bruno](https://github.com/usebruno/bruno),
the open source offline API client, by CEN Nouvelle-Aquitaine
(tested against Central v2024.3.1).
It groups requests into folders for authentication, instance, Projects,
Forms and Datasets (Entity Lists), with an environment file for the server
URL and credentials.
Like the Pentaho tutorial above, it addresses the GUI audience: click through
requests to explore the REST API interactively, test endpoints, and inspect
raw responses before scripting them with `ruODK` or `pyODK`.
...
| Test coverage            | ![tic](https://github.com/ropensci/ruODK/workflows/tic/badge.svg)  [![Build status](https://ci.appveyor.com/api/projects/status/1cs19xx0t64bmd2q/branch/main?svg=true)](https://ci.appveyor.com/project/florianm/ruodk/branch/main) [![codecov](https://codecov.io/gh/ropensci/ruODK/branch/main/graph/badge.svg)](https://codecov.io/gh/ropensci/ruODK) | ❌ | [![Travis-CI Build Status](https://travis-ci.org/edwindj/cbsodataR.png?branch=master)](hhttps://app.travis-ci.com/edwindj/cbsodataR) [![AppVeyor Build Status](https://ci.appveyor.com/api/projects/status/github/edwindj/cbsodatar?branch=master)](https://ci.appveyor.com/project/edwindj/cbsodatar) | ❌ | ❌ |
| Targets ODK Central | ✅ | ❌ | ❌ | ❌ | ❌ |
